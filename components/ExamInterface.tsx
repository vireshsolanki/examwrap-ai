  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] max-h-[calc(100vh-64px)] overflow-hidden fade-in relative bg-background">
      ...
      <div className="flex-grow flex flex-col lg:flex-row overflow-hidden relative">
        ...
        {/* Left: Document View */}
        <div className="lg:w-1/2 p-6 md:p-10 overflow-y-auto border-b lg:border-b-0 lg:border-r border-white/5 bg-black/40 custom-scrollbar scroll-smooth">
             <div className="max-w-2xl mx-auto lg:mx-0">
                <div className="flex items-center justify-between mb-8">
                    <div className="flex gap-2">
                        <span className="px-2 py-1 rounded-sm text-[10px] font-bold uppercase tracking-widest bg-white/5 border border-white/10 text-text-secondary font-mono">
                            {currentQuestion.type}
                        </span>
                        <span className={`px-2 py-1 rounded-sm text-[10px] font-bold uppercase tracking-widest border font-mono
                    ...
                    {!isReviewMode && questionTimeLeft !== null && (
                        <div className={`flex items-center gap-1.5 text-xs font-mono font-bold ${questionTimeLeft < 10 ? 'text-red-500 animate-pulse' : 'text-primary'}`}>
                            <Zap className="w-3.5 h-3.5" />
                            {isTimeUp ? "TIME'S UP" : `${questionTimeLeft}s`}
                        </div>
                    )}
                </div>

                <div className="prose prose-invert prose-lg max-w-none">
                    <h1 className="text-xl md:text-2xl font-medium text-white leading-relaxed font-sans">
                        {currentQuestion.text}
                    </h1>
                </div>
                
                {isReviewMode && (
                    <div className="mt-8 p-6 bg-emerald-900/10 border border-emerald-500/20 rounded-xl relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-1 h-full bg-emerald-500"></div>
                        <div className="flex items-center gap-2 mb-3 text-emerald-400">
                            <Activity className="w-4 h-4" />
                            <h3 className="text-xs font-bold uppercase font-mono tracking-wider">Correct Solution</h3>
                        </div>
                        {currentQuestion.type === QuestionType.MCQ ? (
                            ...
                            <div>
                                <p className="text-xs text-text-secondary mb-1 font-mono">Model Answer:</p>
        ...
        <div className="lg:w-1/2 p-6 md:p-10 overflow-y-auto bg-surface/30 custom-scrollbar scroll-smooth">
            <div className="max-w-2xl mx-auto lg:mx-0">
                    ...
                    {currentQuestion.type === QuestionType.MCQ && currentQuestion.options ? (
                        currentQuestion.options.map((option, idx) => {
                            ...
                            if (isAnswerChecked || isReviewMode) {
                                if (idx === currentQuestion.correctAnswerIndex) {
                                    optionClass = "bg-emerald-500/10 border-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.1)]";
                            ...
                            return (
                                <button
                                    ...
                                    className={`
                                        w-full text-left p-5 rounded-lg border transition-all flex items-center gap-5 group relative active:scale-[0.99] duration-150
                                        ${optionClass}
                                    `}
                                >
                                    <div className={`w-6 h-6 rounded flex items-center justify-center shrink-0 border transition-all font-bold text-xs ${markerClass}`}>
                                        {String.fromCharCode(65 + idx)}
                                    </div>
                                    <span className={`text-base leading-relaxed ${isSelected || ((isAnswerChecked || isReviewMode) && idx === currentQuestion.correctAnswerIndex) ? 'text-white font-medium' : 'text-text-secondary'}`}>
                                        {option}
                                    </span>
                                </button>
                            )
                        ...
                        <div className="relative group">
                            <textarea
                                ...
                                className={`
                                    w-full min-h-[240px] bg-black/30 border rounded-lg p-6 text-white focus:ring-1 focus:ring-primary focus:border-primary outline-none resize-none leading-relaxed font-mono text-sm transition-all
                ...
                {isTimeUp && !isReviewMode && (
                    <div className="mt-6 p-4 bg-red-500/10 border border-red-500/30 rounded-lg text-red-400 text-sm font-medium flex items-center gap-2">
                ...
                {(isAnswerChecked || isReviewMode) && (
                    <div className="mt-8 p-6 rounded-lg bg-secondary/5 border border-secondary/20 animate-in slide-in-from-bottom-4 shadow-lg backdrop-blur-sm relative overflow-hidden">
                         <div className="absolute top-0 left-0 w-full h-0.5 bg-gradient-to-r from-secondary to-transparent"></div>
                        <div className="flex items-center gap-2 mb-3 text-secondary">
                            <HelpCircle className="w-5 h-5" />
                            <h3 className="text-xs font-bold uppercase tracking-widest font-mono">Explanation</h3>
                        </div>
      ...
      {/* Footer */}
      <div className="h-20 border-t border-white/5 bg-background/90 backdrop-blur-md shrink-0 flex items-center justify-center px-6 z-20">
        <div className="w-full max-w-7xl flex justify-between items-center">
             
             <button
                onClick={() => navigateTo(currentIndex - 1)}
                disabled={currentIndex === 0 || isAnalyzing || (isTimeUp && !isReviewMode)}
                className="flex items-center gap-2 px-6 py-2.5 text-text-secondary hover:text-white hover:bg-white/5 border border-transparent hover:border-white/10 rounded-lg transition-all active:scale-95 duration-200 disabled:opacity-30 disabled:active:scale-100 font-mono text-sm"
             >
             ...
             <div className="flex gap-4">
                 {!isReviewMode && (
                    <button
                        ...
                        className="hidden md:block px-6 py-2.5 text-white font-bold bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 rounded-lg transition-all active:scale-95 duration-200 disabled:opacity-50"
                    >
                        Check Answer
                    </button>
                 )}
